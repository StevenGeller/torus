...
pub fn detect_aspect(word: &str) -> Aspect {
    ...
    if w.ends_with("ing") {
        Aspect::Unbounded
    } else if w.ends_with("ed") || matches!(w.as_str(),
        "went" | "came" | "saw" | "knew" | "thought" | "took" | "gave"
        | "made" | "found" | "said" | "told" | "got" | "had" | "did"
        | "was" | "were" | "been" | "spoke" | "wrote" | "drove" | "broke"
        | "chose" | "fell" | "held" | "kept" | "left" | "lost" | "met"
        | "paid" | "ran" | "sat" | "stood" | "understood" | "won" | "wore"
        | "bought" | "brought" | "built" | "caught" | "fought" | "felt"
        | "forgot" | "heard" | "led" | "meant" | "read" | "sent" | "slept"
        | "spent" | "taught" | "threw" | "woke"
    ) {
        Aspect::Bounded
    } else {
        Aspect::Timeless
...
pub fn categorize(word: &str) -> Category {
    ...
    // Negation
    if matches!(base, "not" | "never" | "neither" | "nor" | "nowhere" | "nothing"
    ...
    if base.ends_with("ing") || base.ends_with("ize") || base.ends_with("ify")
        || base.ends_with("ate") {
    ...
    if base.ends_with("tion") || base.ends_with("sion") || base.ends_with("ment")
        || base.ends_with("ness") || base.ends_with("ity") || base.ends_with("ism")
        || base.ends_with("ist") || base.ends_with("ance") || base.ends_with("ence") {
        return Category::Entity;
    }
    if base.ends_with("ous") || base.ends_with("ful") || base.ends_with("less")
        || base.ends_with("able") || base.ends_with("ible") || base.ends_with("ive")
        || base.ends_with("al") || base.ends_with("ial") || base.ends_with("ical") {
...
fn assign_roles(words: &mut [CategorizedWord]) {
    ...
    if let Some(vi) = verb_idx {
        ...
        let copular = matches!(words[vi].word.to_lowercase().as_str(),
...
fn detect_modifiers(words: &mut [CategorizedWord]) {
    let focus_words = ["only", "even", "just", "also", "merely", "simply",
                       "especially", "particularly", "mainly", "mostly"];

    let len = words.len();
    for i in 0..len {
        if words[i].category != Category::Property { continue; }
...
fn dictionary_lookup(word: &str) -> Option<Category> {
    Some(match word {
        ...
        | "more" | "most" | "own" | "same" => Category::Particle,
        ...
        | "across" | "along" | "until" | "since" | "than" | "like"
        | "as" | "per" | "via" => Category::Relation,
        ...
        | "here" | "there" | "now" | "always" | "never" | "ever"
        | "just" | "also" | "very" | "often" | "already"
        | "even" | "only" | "again" | "too" | "quite" | "enough"
        | "well" | "together" | "away" | "back" | "up" | "down"
        | "out" | "off" | "perhaps" | "maybe" | "finally" | "really" => Category::Property,
        ...
        | "arrival" | "departure" | "journey" | "destination" => Category::Entity,
...
pub fn decompose_to_primes(word: &str) -> Vec<SemanticPrime> {
    ...
    match w.as_str() {
        ...
        "love" => vec![p("FEEL", "mental"), p("GOOD", "evaluator"), p("MUCH/MANY", "quantifier")],
        "hate" => vec![p("FEEL", "mental"), p("BAD", "evaluator"), p("MUCH/MANY", "quantifier")],
        "fear" | "afraid" => vec![p("FEEL", "mental"), p("BAD", "evaluator"), p("HAPPEN", "action")],
        "hope" => vec![p("WANT", "mental"), p("GOOD", "evaluator"), p("HAPPEN", "action")],
        "understand" | "understanding" => vec![p("KNOW", "mental"), p("THINK", "mental")],
        "wisdom" => vec![p("KNOW", "mental"), p("GOOD", "evaluator"), p("MUCH/MANY", "quantifier")],
        "beautiful" | "beauty" => vec![p("SEE", "mental"), p("FEEL", "mental"), p("GOOD", "evaluator")],
        "infinite" | "infinity" | "eternal" | "eternity" => vec![p("MUCH/MANY", "quantifier"), p("A LONG TIME", "time"), p("NOT", "logical")],
        "dream" | "dreaming" => vec![p("THINK", "mental"), p("SEE", "mental"), p("NOT", "logical")],
        "circle" | "cycle" | "loop" | "ring" | "torus" => vec![p("LIKE/WAY", "similarity"), p("THE SAME", "determiner"), p("MOVE", "action")],
        "beginning" => vec![p("BEFORE", "time"), p("ALL", "quantifier")],
        "ending" => vec![p("AFTER", "time"), p("ALL", "quantifier")],
        "war" => vec![p("DO", "action"), p("BAD", "evaluator"), p("MUCH/MANY", "quantifier"), p("PEOPLE", "substantive")],
        "peace" => vec![p("NOT", "logical"), p("BAD", "evaluator"), p("FEEL", "mental"), p("GOOD", "evaluator")],
        "home" => vec![p("WHERE/PLACE", "space"), p("LIVE", "life"), p("FEEL", "mental"), p("GOOD", "evaluator")],
        "friend" => vec![p("SOMEONE", "substantive"), p("FEEL", "mental"), p("GOOD", "evaluator")],
        "child" | "children" => vec![p("SOMEONE", "substantive"), p("SMALL", "descriptor"), p("LIVE", "life")],
        "father" | "mother" => vec![p("SOMEONE", "substantive"), p("LIVE", "life"), p("BEFORE", "time")],
        "god" => vec![p("SOMEONE", "substantive"), p("BIG", "descriptor"), p("ABOVE", "space"), p("CAN", "logical"), p("ALL", "quantifier")],
        "consciousness" | "awareness" => vec![p("KNOW", "mental"), p("THINK", "mental"), p("I", "substantive")],
        "world" | "universe" => vec![p("SOMETHING", "substantive"), p("BIG", "descriptor"), p("ALL", "quantifier")],
        "sun" | "star" => vec![p("SOMETHING", "substantive"), p("BIG", "descriptor"), p("ABOVE", "space"), p("SEE", "mental")],
        "water" | "sea" | "ocean" | "river" => vec![p("SOMETHING", "substantive"), p("MOVE", "action")],
        "fire" => vec![p("SOMETHING", "substantive"), p("FEEL", "mental"), p("BAD", "evaluator"), p("SEE", "mental")],
        "earth" | "ground" => vec![p("SOMETHING", "substantive"), p("BIG", "descriptor"), p("BELOW", "space")],
        "sky" => vec![p("SOMETHING", "substantive"), p("BIG", "descriptor"), p("ABOVE", "space")],
        "night" => vec![p("WHEN/TIME", "time"), p("NOT", "logical"), p("SEE", "mental")],
        ...
        "power" | "force" | "strong" => vec![p("CAN", "logical"), p("DO", "action"), p("MUCH/MANY", "quantifier")],
        "free" | "freedom" => vec![p("CAN", "logical"), p("DO", "action"), p("WANT", "mental")],
        "journey" | "travel" => vec![p("MOVE", "action"), p("FAR", "space"), p("A LONG TIME", "time")],
        "arrival" => vec![p("MOVE", "action"), p("HERE", "space"), p("NOW", "time")],
        "music" | "song" => vec![p("HEAR", "mental"), p("FEEL", "mental"), p("GOOD", "evaluator")],
        "story" => vec![p("SAY", "speech"), p("WORDS", "speech"), p("HAPPEN", "action")],
        "truth" => vec![p("TRUE", "speech"), p("KNOW", "mental")],
        "lie" | "lying" => vec![p("SAY", "speech"), p("NOT", "logical"), p("TRUE", "speech")],
        "remember" | "memory" => vec![p("THINK", "mental"), p("KNOW", "mental"), p("BEFORE", "time")],
        "forget" | "forgot" => vec![p("NOT", "logical"), p("KNOW", "mental"), p("BEFORE", "time")],
        "teach" | "teaching" => vec![p("SAY", "speech"), p("KNOW", "mental"), p("SOMEONE", "substantive")],
        "learn" | "learning" => vec![p("KNOW", "mental"), p("BEFORE", "time"), p("NOT", "logical")],
        "create" | "creation" => vec![p("DO", "action"), p("SOMETHING", "substantive"), p("BEFORE", "time"), p("NOT", "logical")],
        "destroy" => vec![p("DO", "action"), p("NOT", "logical"), p("THERE IS", "existence")],
        "birth" | "born" => vec![p("LIVE", "life"), p("BEFORE", "time"), p("NOT", "logical")],
        "grow" | "growing" | "growth" => vec![p("LIVE", "life"), p("BIG", "descriptor"), p("MUCH/MANY", "quantifier")],
        ...
        "sad" | "sadness" | "sorrow" => vec![p("FEEL", "mental"), p("BAD", "evaluator"), p("WANT", "mental")],
        "anger" | "angry" => vec![p("FEEL", "mental"), p("BAD", "evaluator"), p("WANT", "mental"), p("DO", "action")],
        "soul" | "spirit" => vec![p("SOMETHING", "substantive"), p("INSIDE", "space"), p("LIVE", "life"), p("FEEL", "mental")],
        "mind" => vec![p("SOMETHING", "substantive"), p("THINK", "mental"), p("INSIDE", "space")],
        "heart" => vec![p("SOMETHING", "substantive"), p("FEEL", "mental"), p("INSIDE", "space"), p("BODY", "substantive")],
        "voice" => vec![p("SAY", "speech"), p("BODY", "substantive"), p("HEAR", "mental")],
        "eye" | "eyes" => vec![p("SEE", "mental"), p("BODY", "substantive")],
        "hand" | "hands" => vec![p("DO", "action"), p("BODY", "substantive")],
        "face" => vec![p("SEE", "mental"), p("SOMEONE", "substantive"), p("BODY", "substantive")],
        "king" | "queen" => vec![p("SOMEONE", "substantive"), p("BIG", "descriptor"), p("ABOVE", "space"), p("PEOPLE", "substantive")],
        "death" => vec![p("DIE", "life"), p("HAPPEN", "action")],
        "sleep" | "sleeping" => vec![p("NOT", "logical"), p("KNOW", "mental"), p("BODY", "substantive")],
        "walk" | "walking" | "run" | "running" => vec![p("MOVE", "action"), p("BODY", "substantive")],
        "fly" | "flying" => vec![p("MOVE", "action"), p("ABOVE", "space")],
        "fall" | "falling" => vec![p("MOVE", "action"), p("BELOW", "space")],
        "give" | "giving" => vec![p("DO", "action"), p("HAVE", "existence"), p("SOMEONE", "substantive")],
        "take" | "taking" => vec![p("DO", "action"), p("HAVE", "existence"), p("I", "substantive")],
        "open" | "opening" => vec![p("DO", "action"), p("CAN", "logical"), p("INSIDE", "space")],
        "closing" => vec![p("DO", "action"), p("NOT", "logical"), p("CAN", "logical"), p("INSIDE", "space")],
        "begin" | "start" => vec![p("BEFORE", "time"), p("ALL", "quantifier")],
        "end" | "finish" => vec![p("AFTER", "time"), p("ALL", "quantifier")],
        "wait" | "waiting" => vec![p("NOT", "logical"), p("DO", "action"), p("WHEN/TIME", "time")],
        "search" | "searching" | "seek" => vec![p("WANT", "mental"), p("SEE", "mental"), p("SOMETHING", "substantive")],
...
fn default_primes(word: &str) -> Vec<SemanticPrime> {
    ...
    let domain_primes = [
        p("THINK", "mental"), p("WHERE/PLACE", "space"),
        p("WHEN/TIME", "time"), p("THERE IS", "existence"),
        p("LIVE", "life"), p("FEEL", "mental"),
        p("MOVE", "action"), p("SEE", "mental"),
    ];
...
fn try_morphological(word: &str) -> Option<Vec<SemanticPrime>> {
    let w = word.to_lowercase();
    if w.len() < 5 { return None; } // too short to strip meaningfully
    ...
    let prefix_rules: &[(&str, usize, &[(&str, &str)])] = &[
        ...
        ("pre",   3, &[("BEFORE", "time")]),
        ("over",  3, &[("MUCH/MANY", "quantifier"), ("ABOVE", "space")]),
        ("under", 3, &[("BELOW", "space")]),
    ...
    for (prefix, min_len, extra) in prefix_rules {
        if w.starts_with(prefix) && w.len() >= prefix.len() + min_len {
            ...
            let is_default = primes.len() <= 2 && primes.iter().any(|p|
                p.prime == "SOMETHING" || p.prime == "DO" || p.prime == "LIKE/WAY"
            );
            if !is_default {
        ...
        if w.ends_with(suffix) && w.len() >= suffix.len() + min_len {
            ...
            for candidate in spelling_variants(root) {
                ...
                let is_default = primes.len() == 1 && (
                    primes[0].prime == "SOMETHING" || primes[0].prime == "DO"
    ...
    for i in 3..w.len().saturating_sub(2) {
        ...
        if dictionary_lookup(left).is_some() && dictionary_lookup(right).is_some() {
            ...
            for rp in right_primes {
                if !primes.iter().any(|p| p.prime == rp.prime && p.domain == rp.domain) {
                    primes.push(rp);
                }
            }
            return Some(primes);
